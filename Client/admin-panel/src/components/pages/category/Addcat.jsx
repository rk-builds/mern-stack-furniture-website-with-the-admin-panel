...
const pageName = [
  { label: "category", href: "/category/add" },
  { label: "Add", href: "/category/add" }
];
...
export default function Addcat() {
  ...
  let saveCategory = (e) => {
    ...
    if (id) {
      // UPDATE
      axios.post(`${apiBaseUrl}category/update/${id}`, formData)
        .then(res => res.data)
        .then(finalRes => {
    ...
    if (id) {
      axios.get(`${apiBaseUrl}category/edit-category/${id}`)
        ...
        .then((finalRes) => {
          ...
          let { categoryName, categoryOrder, categoryImage } = finalRes.categoryData
          setFormValue({
            categoryName,
            categoryOrder

          })
          setImage(finalRes.staticPath + categoryImage);
  ...
  return (
    <>
      ...
      <div className='min-h-screen max-w-[1050px] mx-auto p-5 '>
        <div className=''>
          <h3 className="text-[20px] font-semibold bg-slate-100 py-2 px-3 rounded-t-md border border-slate-400">Add Category</h3>
        </div>
        <div className='rounded-b-md border border-slate-400 border-t-0 p-5'>
          <form action="" className='' onSubmit={saveCategory}>
            <div className='grid grid-cols-[35%_auto]'>
              ...
              <div>
                <div className='mb-5 mx-auto'>
                  ...
                  <input type="text" value={formValue.categoryName} onChange={handleChange} className='text-[19px] border-2 shadow-sm border-gray-300 text-gray-900 text-sm rounded-lg block w-full py-2 px-2' name="categoryName" placeholder="Parent category Name" />
                </div>

                <div className='mb-5 mx-auto'>
                  <label className='block  text-md font-medium  text-gray-900'>Order</label>
                  <input type="Number" value={formValue.categoryOrder} onChange={handleChange} className='text-[19px] border-2 shadow-sm border-gray-300 text-gray-900 text-sm rounded-lg block w-full py-2 px-2' name="categoryOrder" min={1} placeholder="Enter Order" />
                </div>
              </div>
