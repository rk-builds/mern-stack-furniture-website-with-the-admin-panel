...
const pageName = [
  { label: "slider", href: "/slider/view" },
  { label: "View", href: "/slider/view" }
];
...
export default function ViewSlider() {
  ...
  let getAllChecked = (e) => {
    if (e.target.checked) {
      let finalRes = sliderData.map((v) => v._id)
      setIds(finalRes)
    }
    else {
      setIds([])
  ...
  let multiDelete = async () => {
    try {
      ...
      if (result.isConfirmed) {
        ...
        if (ids.length >= 1) {
          await axios.post(`${apiBaseUrl}slider/multi-delete`, { ids });
          getsliderView();
  ...
  let statusUpdate = () => {
    ...
    if (ids.length > 0) {
      axios.post(`${apiBaseUrl}slider/status-update`,
        {
          ids
        }
      )
  ...
  return (
    ...
    <ToastContainer />
      ...
      <section className='p-10 min-h-screen '>
        <Tableform heading="View Slider" onMultiDelete={multiDelete} onStatusUpdate={statusUpdate} />
            ...
            <tbody>
              {
                sliderData.length >= 1 ?
                  sliderData.map((sliderobj, index) => {
                    return (
                      <tr className='hover:bg-gray-50' key={sliderobj._id}>
                        {/* {console.log(ids)} */}
                        <td className='ps-4 py-5'><input type="checkbox" value={sliderobj._id} onChange={getChecked} checked={ids.includes(sliderobj._id)} /></td>
                        <td>{(currentPage - 1) * limit + index + 1}</td>
                        <td className='px-2 py-5 text-base font-semibold'>{sliderobj.sliderTitle}</td>
                        <td className='px-2 py-5'>
                          <img src={staticPath + sliderobj.sliderImage} width={50} alt="" />
                        </td>
                        <td className='px-2 py-5'>{sliderobj.sliderOrder}</td>
                        <td className='px-2 py-5'>
                        ...
                        <td className='px-2 py-5'>
                          <Link to={`/edit-slider/${sliderobj._id}`}>
